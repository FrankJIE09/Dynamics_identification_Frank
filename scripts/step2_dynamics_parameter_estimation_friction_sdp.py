# -*- coding: utf-8 -*-
"""
Step2 动力学参数辨识（含摩擦）- Python 版（SDP 惯量约束）
模型: τ = Y(q,dq,ddq)*θ + Fv*dq + Fc*sign(dq)，其中 Fv/Fc 为 7 维（每关节粘滞/库伦系数）。
流程：读配置 → 加载 URDF 与 CSV → 构造 W=[Y,D_visc,D_coul]、tau →
优先用 SDP（质量+惯量正定+三角不等式）求解 φ=[θ,Fv,Fc]；否则 QP（仅质量）或 Ridge+投影。
SDP/QP 分支下 θ 已满足物理约束，无需后验投影，刚性差从根上避免。
"""
...
def solve_ridge_sdp_friction_stage2_i_com(
    W_all: np.ndarray,
    tau_all: np.ndarray,
    phi_prior: np.ndarray,
    L_diag: np.ndarray,
    phi_stage1: np.ndarray,
    n_links: int,
    I_eps: float,
) -> np.ndarray | None:
    """方案2 第二阶段：固定 (m, mc)，以 I_com 为变量，约束 I_com >> I_eps*I，最小化拟合+正则。
    用 I^o = I_com + S(m,mc) 代入回归，输出为 84 维 phi = [θ(I^o), Fv, Fc]，保证质心处惯量正定。
    """
    if not CVXPY_AVAILABLE or n_links <= 0 or phi_stage1.size != 84:
        return None
    n_phi = 84
    n_params = 70
    if W_all.shape[1] != n_phi or W_all.shape[0] != tau_all.size or L_diag.size != n_phi:
    ...
    # 从 stage1 取出 (m, mc)，计算每连杆 S_j = (1/m)*((mc·mc)*I - mc*mc')
    m_fix = np.array([phi_stage1[10 * j] for j in range(n_links)], dtype=float)
    mc_fix = np.array([phi_stage1[10 * j + 1 : 10 * j + 4] for j in range(n_links)], dtype=float)
    S_list = [_compute_S_from_mc(m_fix[j], mc_fix[j]) for j in range(n_links)]

    # 变量：每连杆 I_com_j (3x3 对称)，Fv(7)，Fc(7)
    I_com_cvx = [cp.Variable((3, 3), symmetric=True) for _ in range(n_links)]
    Fv = cp.Variable(7)
    Fc = cp.Variable(7)

    # theta_70 = (m_fix, mc_fix, I^o) 其中 I^o_j = I_com_j + S_j，Pinocchio 顺序 6 元 (0,0),(0,1),(1,1),(0,2),(1,2),(2,2)
    theta_70_parts = []
    for j in range(n_links):
        theta_70_parts.append(phi_stage1[10 * j])
        theta_70_parts.append(phi_stage1[10 * j + 1])
        theta_70_parts.append(phi_stage1[10 * j + 2])
        theta_70_parts.append(phi_stage1[10 * j + 3])
        I_o_j = I_com_cvx[j] + S_list[j]
        theta_70_parts.append(I_o_j[0, 0])
        theta_70_parts.append(I_o_j[0, 1])
        theta_70_parts.append(I_o_j[1, 1])
        theta_70_parts.append(I_o_j[0, 2])
        theta_70_parts.append(I_o_j[1, 2])
        theta_70_parts.append(I_o_j[2, 2])
    theta_70 = cp.hstack(theta_70_parts)
    phi = cp.hstack([theta_70, Fv, Fc])

    obj = 0.5 * cp.sum_squares(W_all @ phi - tau_all) + 0.5 * cp.sum(
        cp.multiply(L_diag, cp.square(phi - phi_prior))
    )
    constraints = [I_com_cvx[j] >> I_eps * np.eye(3) for j in range(n_links)]

    print("  [SDP Stage2 I_com] 变量: I_com 7x(3x3 对称), Fv(7), Fc(7); 约束: I_com_j >> I_eps*I (质心处惯量正定)")
    prob = cp.Problem(cp.Minimize(obj), constraints)
    try:
        prob.solve(solver=cp.SCS, verbose=True)
        if prob.status not in (cp.OPTIMAL, cp.OPTIMAL_INACCURATE):
            print("  [SDP Stage2 I_com] 求解状态: {}".format(prob.status))
            return None
        # 组装 phi 数值：theta_70 用 I_com.value + S
        theta_70_val = np.zeros(70)
        for j in range(n_links):
            theta_70_val[10 * j : 10 * j + 4] = phi_stage1[10 * j], phi_stage1[10 * j + 1], phi_stage1[10 * j + 2], phi_stage1[10 * j + 3]
            I_o_j_val = I_com_cvx[j].value + S_list[j]
            theta_70_val[10 * j + 4] = I_o_j_val[0, 0]
            theta_70_val[10 * j + 5] = I_o_j_val[0, 1]
            theta_70_val[10 * j + 6] = I_o_j_val[1, 1]
            theta_70_val[10 * j + 7] = I_o_j_val[0, 2]
            theta_70_val[10 * j + 8] = I_o_j_val[1, 2]
            theta_70_val[10 * j + 9] = I_o_j_val[2, 2]
        return np.hstack([theta_70_val, Fv.value, Fc.value])
    except Exception as e:
        print("  [SDP Stage2 I_com] 求解异常: {}".format(e))
        return None
...
def _compute_S_from_mc(m: float, mc: np.ndarray) -> np.ndarray:
    """S = (1/m)*((mc·mc)*I - mc*mc')，平行轴公式中从 I_com 到 I^o 的附加项。返回 3x3。"""
    mc = np.asarray(mc, dtype=float).ravel()[:3]
    m = max(float(m), 1e-12)
    r2 = float(np.dot(mc, mc))
    return (r2 * np.eye(3) - np.outer(mc, mc)) / m
...
def estimate_dynamics_parameters(data_file: str, urdf_file: str, cfg: dict) -> None:
    ...
    # 方案 C 优先：SDP（质量+惯量正定+三角不等式）→ QP（仅质量）→ Ridge+投影
    use_sdp = (
        cfg.get("use_sdp_inertia", True)
        and CVXPY_AVAILABLE
        and n_params == 70
    ...
    if use_sdp:
        # 方案 C：SDP 求解 φ=[θ,Fv,Fc]。约束为质量下界 + 质心处惯量 I_com 正定（关节处 I^o 不约束）。
        # 参数含义：W_all 扩展回归矩阵，tau_all 实测力矩堆叠，phi_prior 先验（θ 取 URDF），
        # L_diag 正则对角（θ 用 lam、摩擦用 lam_friction），n_params=70、n_links=7，
        # m_min 质量下界，I_eps 惯量最小特征值下界，I_trace_min 保留兼容（SDP 内未用，写 URDF 前投影可能用）。
        phi, used_i_com = solve_ridge_sdp_friction(
            W_all, tau_all, phi_prior, L_diag, n_params, n_links, m_min,
            cfg["I_eps"], cfg["I_trace_min"],
        )
        if phi is not None:
            used_constrained_solve = True
            if used_i_com:
                print(f"  使用 SDP（质量+质心处I_com正定）求解 φ=[θ,Fv,Fc]: lambda_rel={lambda_rel}, lam_friction={lam_friction:.2e}，无需投影")
            else:
                print(f"  使用 SDP（仅质量约束，I_com 未通过 DCP/求解）求解 φ=[θ,Fv,Fc]: lambda_rel={lambda_rel}, lam_friction={lam_friction:.2e}，无需投影")
            # 方案2：以 I_com 为变量再解一阶，保证输出惯量在质心处正定
            if cfg.get("use_sdp_i_com_stage2", True) and n_links == 7 and n_params == 70:
                print("  [方案2] 第二阶段：固定 (m,mc)，以 I_com 为变量约束 I_com>>I_eps*I...")
                phi_stage2 = solve_ridge_sdp_friction_stage2_i_com(
                    W_all, tau_all, phi_prior, L_diag, phi, n_links, cfg["I_eps"],
                )
                if phi_stage2 is not None:
                    phi = phi_stage2
                    print("  [方案2] 第二阶段完成，输出 θ 对应质心处惯量 I_com 正定")
                else:
                    print("  [方案2] 第二阶段未成功，沿用第一阶段结果")
    ...
    # 输出 SDP/约束求解结果：逐连杆 θ 与惯量矩阵，验证正定（Pinocchio 顺序 Ixx,Ixy,Iyy,Ixz,Iyz,Izz）
    if used_constrained_solve and n_links > 0 and n_params == 10 * n_links:
        print("\n  [SDP/约束求解] θ 逐连杆结果（用于写 URDF 的惯量）:")
        for j in range(n_links):
            base = 10 * j
